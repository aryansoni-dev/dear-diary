# DearDiary Database Notes

## Journal Entry Tags

Tags are stored directly on `journal_entries` for the MVP.
...
The journal sync RPC must also include `tags` so local-first pushes write the array to Supabase.
